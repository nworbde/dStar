...
subroutine conductivity(rho,T,chi,Gamma,eta,mu_e,ionic,kappa,which_ee,which_eQ,K_components)
    ...
    subroutine clear_kappa()
        ...
        kappa% nQ = 0.0
        kappa% np = 0.0
        kappa% electron_total = 0.0
        kappa% neutron_total = 0.0
...
function n_imp(nn, nion, temperature, ionic, ierr) result(nu)
	...
	call dop853_work_sizes(num_integration_variables,num_integration_variables,liwork,lwork)
	...
	call dop853(n,structure_factor_impurity,kn_start,y,kn_end,h,int_default_max_step_size,int_default_max_steps, &
    ...
    mnstar = Mneutron   
   	Efn = 15.1*mev_to_ergs*(ionic% Yn)**(2.0/3.0)*(nn*amu/ionic% Yn/1.0E14)**(2.0/3.0)* &
...
function n_phonon(nn, nion, temperature, ionic, ierr) result(nu)
	...
	call dop853(n,structure_factor_phonon,kn_start,y,kn_end,h,int_default_max_step_size,int_default_max_steps, &
			& rtol,atol,itol, solout, iout, work, lwork, iwork, liwork,  &
			&	num_rpar, rpar, num_ipar, ipar, lout, idid)
