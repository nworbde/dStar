program run_dStar
    ...
    if (check_okay% raised(ierr,'set command argument shallow heating')) stop

    call command_arg_set( &
        & 'Mcore','core mass',ierr, &
        & flag='M',takes_parameter=.TRUE.)
    if (check_okay% raised(ierr,'set command argument core mass')) stop

    call command_arg_set( &
        & 'Rcore','core radius',ierr, &
        & flag='R',takes_parameter=.TRUE.)
    if (check_okay% raised(ierr,'set command argument core radius')) stop
