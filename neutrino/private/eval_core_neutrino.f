...
contains
	...
	subroutine emissivity(nn,np,T,Tcs,include_channel,eps)
		...
		call clear_epsilon		
		if (include_channel(icore_nu_brem))  &
				& call nubrems(nn,np,T9,vps,vns,vnt,eps% brem,eps% brem_np, eps% brem_pp, eps% brem_nn)
		
		if (include_channel(icore_nu_mUrca))  &
				& call mUrca(nn,np,T9,vps,vns,vnt,eps% mUrca, eps% mUrca_p, eps% mUrca_n)

		if (include_channel(icore_nu_dUrca)) call dUrca(nn,np,T9,vps,vns,vnt,eps% dUrca)

		if (include_channel(icore_nu_PBF))  &
				& call nupbf(nn,np,T9,vps,vns,vnt,eps% PBF, eps% PBF_n, eps% PBF_p)
		...
		subroutine clear_epsilon()
			eps% total = 0.0
			eps% brem = 0.0; eps% brem_np = 0.0; eps% brem_pp = 0.0; eps% brem_nn = 0.0
			eps% mUrca = 0.0; eps% mUrca_p = 0.0; eps% mUrca_n = 0.0
			eps% dUrca = 0.0
			eps% PBF = 0.0; eps% PBF_n = 0.0; eps% PBF_p = 0.0
	...
	subroutine nubrems(nn,np,T9,vps,vns,vnt,eps,eps_np,eps_pp,eps_nn)
		...
		real(dp), parameter :: Qnn = 7.4e19, Qnp = 1.5e20, Qpp = 7.4e19
		real(dp) :: T98, rbpnp, rbppp, rbnns, rbnnt, rbnnn, rbnp, rda, rdb, rd, ap, bp, cp, dp, cn, dn
		...
		if (vps > 0.0) then
			...
			dp = 0.7333 + sqrt(0.2667**2+(0.1678*vps)**2)
			...
			rbppp = 0.5*(cp**2*exp(4.228-sqrt(4.228**2+(2*vps)**2)) & 
						 &	+ dp**7.5*exp(7.762-sqrt(7.762**2+(3.0*vps)**2)))
		end if
	...
	! modified Urca
		...
		real(dp), parameter :: Qn = 8.55e21, Qp = 8.53e21
	...
	subroutine nupbf(nn,np,T9,vps,vns,vnt,eps,eps_n,eps_p)
		...
		real(dp), parameter :: Q = 1.170e21*3.0, a_cns=1.0, a_cnt=3.18
		real(dp) :: T97
	...
	subroutine effective_masses(n,x, mn,mp)
		...
		real(dp), parameter ::  p3 = 89.8	! MeV fm^5^
		real(dp), parameter ::  p4 = 0.457	! fm^3^
		real(dp), parameter ::  p5 = -59.0	! MeV fm^5^	
		real(dp) :: t1, t2, t3
